...
class Autoencoder(nn.Module):
    def __init__(self,img_channels,latent_channels,masked=False,loss = "Focal"):
        ...
        self.decoder = Decoder(img_channels,latent_channels)
        self.masked = masked
        if loss == "Focal":
            ...
            for i in range(bs):
                for _ in range(random_box_num):
                    ...
                    if self.loss_name == "Focal":
                        ...
                        orig[i,:,center_x:center_x+random_box_x,center_y:center_y+random_box_y] = one_hot_tensor
        ...
        for i, samples in enumerate(databar):
            ...
            if ((epoch+1) % 10 == 0 or epoch==0)and i==t:
                with torch.no_grad():
                    ...
                    fake_real=torch.cat([data.cpu(),break_data.cpu(),fake_data.cpu()],dim=0)
                    fake_real = tensor2png(fake_real,batch_size*3)
                    save_image(fake_real, os.path.join("/home/sadong/refine_ddpm/resnet_VAE/mask_cuda0/img", 'train_fake_real_{}.png'.format(epoch+1)), nrow=4,cmap='gary',)
                    ...
                    img=autoencoder(break_data).cpu()
                    break_data = break_data.cpu()                    
                    data = data.cpu()
                    fake_real=torch.cat([data,break_data,img],dim=0)
                    fake_real = tensor2png(fake_real,3)
                    save_image(fake_real, os.path.join("/home/sadong/refine_ddpm/resnet_VAE/mask_cuda0/img", 'test_fake_real_{}.png'.format(epoch+1)), nrow=4)
                    torch.save(autoencoder.state_dict(),"/home/sadong/refine_ddpm/resnet_VAE/mask_cuda0/model/epoch{}.pkl".format(epoch+1))
