...
def main():
    ...
    for epoch in range(0,epochs):
        ...
        for i, samples in enumerate(databar):
            ...
            loss.backward()
            optimizer.step()
            if (epoch+1) % 10 == 0 and i==t:
                with torch.no_grad():
                    fake_data = autoencoder(data)
                    fake_real=torch.cat([data.cpu(),fake_data.cpu()],dim=0)
                    fake_real = tensor2png(fake_real,batch_size*2)
                    save_image(fake_real, os.path.join("/home/sadong/refine_ddpm/resnet_VAE_newdata/focal_cuda0/img", 'train_fake_real_{}.png'.format(epoch+1)), nrow=4)
                    (pop, dem, lan),data = next(it)
                    data = data.to(device)
                    img=autoencoder(data).cpu()
                    data = data.cpu()
                    fake_real=torch.cat([img,data],dim=0)
                    fake_real = tensor2png(fake_real,2)
                    save_image(fake_real, os.path.join("/home/sadong/refine_ddpm/resnet_VAE_newdata/focal_cuda0/img", 'test_fake_real_{}.png'.format(epoch+1)), nrow=4)
                    torch.save(autoencoder.state_dict(),"/home/sadong/refine_ddpm/resnet_VAE_newdata/focal_cuda0/model/epoch{}.pkl".format(epoch+1))
