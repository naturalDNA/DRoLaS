...
from nn1 import (avg_pool_nd, conv_nd, linear, normalization,
...
class AttentionBlock1(nn.Module):
    ...
    def __init__(
        ...
        self.to_kv = conv_nd(1, label_nc, channels * 2, 1)
    ...
    def _forward(self, x, cond):
        ...
        b, c, *spatial = x.shape
        b1, c1, *spatial1 = cond.shape        
        x = x.reshape(b, c, -1)
        cond = cond.reshape(b, c1, -1)
        kv = self.to_kv(cond)
...
class SpatialCrossAttention(nn.Module):
    def __init__(self, norm_nc, label_nc):
        ...
        self.to_q = nn.Conv2d(norm_nc, norm_nc//2, kernel_size=1, stride=1, padding=0)
        self.to_k = nn.Conv2d(label_nc, norm_nc//2, kernel_size=1, stride=1, padding=0)
        self.to_v = nn.Conv2d(label_nc, norm_nc//2, kernel_size=1, stride=1, padding=0)
        
        self.to_out = nn.Conv2d(norm_nc//2, norm_nc, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, segmap):
        q = self.relu(self.to_q(x))
        k = self.relu(self.to_k(segmap))
        v = self.relu(self.to_v(segmap))      
        ...
        attn_map = (q.size(-1)**-.5) * attn_map  
        attn_map = F.softmax(attn_map, dim=-1)

        context = torch.matmul(attn_map, v) 
        context = context.permute(0, 2, 1).contiguous() 
        context = context.reshape(x.size(0), -1, *x.shape[2:]) 

        output=self.to_out(context)

        result = x + torch.sigmoid(self.weight)*output
...
class SPADEGroupNorm(nn.Module):
    def __init__(self, norm_nc, label_nc, eps = 1e-5, has_attn = False):
        super().__init__()
        ...
        self.spatialAtt = SpatialCrossAttention(norm_nc, label_nc)
        # if has_attn:
        #     self.spatialAtt = SpatialCrossAttention(norm_nc, label_nc)
        #     #self.spatialAtt = AttentionBlock1(norm_nc, label_nc)
        # else:
    ...
    def forward(self, x, segmap):
        ...
        if self.has_attn:
            x2 = self.spatialAtt(self.act(x1),segmap)
        else:
            x2=x1
...
class SDMResidualBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, time_channels: int = 1, n_groups: int = 16, c_channels: int = 16,has_attn=False):
        """
        * `in_channels` is the number of input channels
        * `out_channels` is the number of input channels
        * `time_channels` is the number channels in the time step ($t$) embeddings
        * `n_groups` is the number of groups for [group normalization](../../normalization/group_norm/index.html)
        ...
        # Group normalization and the first convolution layer
        #self.norm1 = nn.GroupNorm(n_groups, in_channels)
        self.norm1 = SPADEGroupNorm(in_channels, c_channels,has_attn=has_attn)
        self.act1 = Swish()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=(3, 3), padding=(1, 1))

        # Group normalization and the second convolution layer
        #self.norm2 = nn.GroupNorm(n_groups, out_channels)
        self.norm2 = SPADEGroupNorm(out_channels, c_channels,has_attn=has_attn)
        self.act2 = Swish()
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=(3, 3), padding=(1, 1))
...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, time_channels: int, has_attn: bool):
        ...
        self.res = SDMResidualBlock(in_channels + out_channels, out_channels, time_channels,c_channels=out_channels,has_attn=has_attn)
        if has_attn:
            self.attn = AttentionBlock(out_channels)
        else:
            self.attn = nn.Identity()
