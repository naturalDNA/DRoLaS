...
class DenoiseDiffusion:
    ...
    def p_sample(self, xt: torch.Tensor, t: torch.Tensor,condition:torch.tensor):
        """
        从 xt 得到xt-1
    ...
    def loss(self, x0: torch.Tensor, noise: Optional[torch.Tensor] = None,condition:torch.tensor=None,region=None,weight=[0.2,0.4,0.3,0.4]):
        """
        return:loss
        """
        ...
        loss_orig=F.mse_loss(noise, eps_theta,reduction='none')

        #weight_region=weight[0]*region[0:32,:,:,:]+weight[1]*region[32:64,:,:,:]+weight[2]*region[64:96,:,:,:]+weight[3]*region[96:128,:,:,:]
        weight_region=weight[0]*region[0:32,:,:,:]+weight[1]*region[32:64,:,:,:]
        #weight_region=0
        loss = torch.mul(loss_orig,(1+weight_region)).mean([1, 2, 3])
        #loss = torch.mul(loss_orig,(1+weight)).mean([1, 2, 3])
    ...
    def classifire_p_sample_ddim(self,
        ...
        for i in tqdm(range(1, ddim_step + 1),
            ...
            if scale==1:
                ...
                condition1=torch.zeros_like(condition)
                eps_theta1 = net(x, t_tensor,condition=condition)
                #eps_theta2 = net(x, t_tensor,condition=None)
                eps_theta2 = net(x, t_tensor,condition=condition1)
